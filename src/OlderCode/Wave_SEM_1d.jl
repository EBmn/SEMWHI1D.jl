...
mutable struct SEM_Wave
    ...
    function SEM_Wave(
        ...
        #fill G and inverseM with the correct elements
        #=
        for j = 1:PointsPerElement
            for m = 1:PointsPerElement
                for l = 1:PointsPerElement
                    G[j, m] = G[j, m] + QuadWeights[l]*LagrangeDer(QuadPoints, m, l, 0.0001)*LagrangeDer(QuadPoints, j, l, 0.0001)
                end
            end
        end
        =#
        ...
        for k = 1:length(nodes)-1
            ...
            start = (k-1)*(PointsPerElement-1)

            delta_x_k = nodes[k+1] - nodes[k]

            inverseM[(start + 1):(start + PointsPerElement)] += QuadWeights*delta_x_k/2
    ...
    for k = 1:length(simul.nodes)-1 #for each element...
        ...
        if (stepnumber == 1) #the first step is different
            ...
            for j = 2:simul.PointsPerElement-1      #sort out the interior points
                ...
                simul.uNext[(k-1)*(simul.PointsPerElement-1) + j] = simul.uNow[j + (k-1)*(simul.PointsPerElement-1)] + 0.5*simul.timestep^2*(LaplaceCalculation(simul.uNow, j, k) + ForcingTerm(j, k, stepnumber))  
            ...
            if k != (length(simul.nodes)-1)
                ...
                simul.uNext[k*(simul.PointsPerElement-1) + 1] = simul.uNow[k*(simul.PointsPerElement-1) + 1] + 0.5*simul.timestep^2*(LaplaceCalculation(simul.uNow, PointsPerElement, k) + ForcingTerm(PointsPerElement, k, stepnumber))
            ...
            for j = 2:simul.PointsPerElement-1      #sort out the interior points
                ...
                simul.uNext[(k-1)*(simul.PointsPerElement-1) + j] = 2*simul.uNow[j + (k-1)*(simul.PointsPerElement-1)] - 
                                                                    simul.uPrev[j + (k-1)*(simul.PointsPerElement-1)] +
                                                                    simul.timestep^2 * LaplaceCalculation(simul.uNow, j, k) + 
                                                                    simul.timestep^2 * ForcingTerm(j, k, stepnumber)
            ...
            if k != length(simul.nodes)-1
                ...
                simul.uNext[k*(simul.PointsPerElement-1) + 1] = 2*simul.uNow[k*(simul.PointsPerElement-1) + 1] - 
                                                                simul.uPrev[k*(simul.PointsPerElement-1) + 1] +
                                                                simul.timestep^2*LaplaceCalculation(simul.uNow, simul.PointsPerElement, k) +
                                                                simul.timestep^2*ForcingTerm(simul.PointsPerElement, k, stepnumber)
